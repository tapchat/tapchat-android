...
public class OpenBufferReceiver extends BroadcastReceiver {

    @Override public void onReceive(Context context, Intent intent) {
        long cid = Long.parseLong(intent.getStringExtra("cid"));
        long bid = Long.parseLong(intent.getStringExtra("bid"));

        // FIXME: Buffer may be archived.

        Context appContext = TapchatApp.get();

        Intent activityIntent = BuffersActivity.createIntent(context, cid, bid);
